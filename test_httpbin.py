...
import bl_search_faiss
from bl_search_faiss.helpers import parse_multi_value_header
...
class HttpbinTestCase(unittest.TestCase):
    ...
    def setUp(self):
        bl_search_faiss.app.debug = True
        self.app = bl_search_faiss.app.test_client()
    ...
    def test_post_body_text(self):
        with open('bl_search_faiss/core.py') as f:
    ...
    def test_digest_auth_with_wrong_password(self):
        ...
        response = self.app.get(
            ...
            environ_base={
                # bl_search_faiss's digest auth implementation uses the remote addr to
                # build the nonce
